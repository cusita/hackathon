...
export default class Balloons{
  ...
  createBalloons(){
    ...
    for(let i=0;i<cfg.balloonCount;i++){
      ...
      this.balloons.set(el, {x,y,baseX:x,baseY:y,phase:Math.random()*Math.PI*2});
    }
    this.startFloat();
  }
  ...
  onClick(e){
    ...
    this.container.dispatchEvent(new CustomEvent('balloon:hit', {detail:{value, x: rect.left+rect.width/2, y: rect.top+rect.height/2}}));
  }

  onPointerMove(e){
    // emitir posición del puntero para que el arco/arrow apunte
    const rect = this.container.getBoundingClientRect();
    this.container.dispatchEvent(new CustomEvent('pointer:pos', {detail:{x: e.clientX - rect.left, y: e.clientY - rect.top}}));
  }
