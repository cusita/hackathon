...
export default class Game{
  constructor(){
    ...
    document.body.addEventListener('pointerdown', ()=>SND.resumeAudio(), {once:true});
  }

  setupEvents(){
    // apuntar con el mouse/pointer
    this.balloonContainer.addEventListener('pointer:pos', e=>this.updateAim(e.detail));
    // impacto sobre globo
    this.balloonContainer.addEventListener('balloon:hit', e=>this.onBalloonHit(e.detail));

    // controles
    document.getElementById('backBtn').addEventListener('click', ()=>this.inputs.backspace());
    document.getElementById('clearBtn').addEventListener('click', ()=>this.inputs.clear());
    ...
    window.addEventListener('keydown', (e)=>{
      ...
      if(e.key==='Tab') { e.preventDefault(); this.toggleActive(); }
    });
  }
  ...
  updateAim(pos){
    ...
    const dx = (pos.x + this.balloonContainer.getBoundingClientRect().left) - centerX;
    const dy = (pos.y + this.balloonContainer.getBoundingClientRect().top) - centerY;
  ...
  onBalloonHit(detail){
    ...
    if(added){
      SND.playPop();
      this.score += cfg.scoring.perHit; this.scoreEl.textContent = String(this.score);
      ...
      if(acc.length >= cfg.confettiThreshold.account){ this.confetti.spawn(window.innerWidth/2, 120); }
      if(Number(tr) >= cfg.confettiThreshold.transferValue){ this.confetti.spawn(window.innerWidth/2, 120); }
    }
  }

  addDigit(d){ return this.inputs.addDigit(d); }

  toggleActive(){ this.inputs.setActive(this.inputs.active==='account' ? 'transfer' : 'account'); }
}
